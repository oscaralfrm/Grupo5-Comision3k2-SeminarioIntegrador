...
function NavbarAlumnoDash() {
  ...
  const { idAlumno, idServicio } = useParams();
  ...
  useEffect(() => {
    const fetchServicios = async () => {
      try {
        ...
        console.error("Error al traer los servicios del alumno:", error);
      }
    };
    fetchServicios();
  }, [idAlumno, idServicio]);
        ...
        <div className="container-fluid d-flex align-items-center">
          ...
          <Navbar.Collapse id="navbarNav" className={dropdownOpen ? "show" : ""}>
            <Nav className="mx-auto d-flex justify-content-center w-100">
              <Nav.Link
                href={`/instructor/${idAlumno}/servicio/${idServicio}/mi-servicio`}
                style={{ color: "white" }}
              >
                Mi Servicio
              </Nav.Link>
